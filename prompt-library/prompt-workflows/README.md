...
RECURSIVE DECISION:
- If quality ≥ 8: Accept final analysis
...
REFINEMENT LOOP:
1. Creator → Critic → Editor → Validator → Stakeholder
...
DECISION TREE:
IF complexity_score ≤ 3: → Simple workflow
IF 3 < complexity_score ≤ 7: → Standard workflow  
IF complexity_score > 7: → Complex workflow
IF special_constraints: → Custom workflow
