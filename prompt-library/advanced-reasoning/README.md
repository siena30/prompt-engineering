...
├── Sub-option A1: {immediate_action}
│   ├── Outcome A1a: {best_case}
│   ├── Outcome A1b: {most_likely}
│   └── Outcome A1c: {worst_case}
└── Sub-option A2: {delayed_action}
    ├── Outcome A2a: {best_case}
    └── Outcome A2b: {worst_case}

BRANCH 2: OPTION B - {option_description}
├── Sub-option B1: {aggressive_approach}
└── Sub-option B2: {conservative_approach}

BRANCH 3: OPTION C - {hybrid_approach}
└── Combined elements from A and B
...
- Source element A ↔ Target element X
- Source element B ↔ Target element Y  
- Source relationship 1 ↔ Target relationship α
- Source constraint P ↔ Target constraint Q
